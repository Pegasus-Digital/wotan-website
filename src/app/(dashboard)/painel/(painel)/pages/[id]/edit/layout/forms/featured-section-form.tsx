...
import { FeaturedSection, Category, Product, Media } from '@/payload/payload-types'
...
interface FeaturedSectionFormProps {
  ...
  onSubmit: (data: FeaturedSectionFormValues & { cards: { image: string }[] }) => void
  isSubmitting?: boolean
}

export function FeaturedSectionForm({ initialData, onSubmit, isSubmitting }: FeaturedSectionFormProps) {
  const [openStates, setOpenStates] = useState<{ [key: string]: boolean }>({})
  const [categories, setCategories] = useState<Category[]>([])
  const [products, setProducts] = useState<Product[]>([])
  const [isLoading, setIsLoading] = useState(false)
  const [error, setError] = useState<string | null>(null)
  const [card0, setCard0] = useState<(Media| string)[]>([initialData?.cards[0].image] )
  const [card1, setCard1] = useState<(Media| string)[]>([initialData?.cards[1].image] )
  const [card2, setCard2] = useState<(Media| string)[]>([initialData?.cards[2].image] )
  const [card3, setCard3] = useState<(Media| string)[]>([initialData?.cards[3].image] )

  useEffect(() => {
    if (initialData?.cards) {
      if (initialData.cards[0]?.image) {
        setCard0([initialData.cards[0].image] )
      }
      if (initialData.cards[1]?.image) {
        setCard1([initialData.cards[1].image] )
      }
      if (initialData.cards[2]?.image) {
        setCard2([initialData.cards[2].image] )
      }
      if (initialData.cards[3]?.image) {
        setCard3([initialData.cards[3].image] )
      }
    }
  }, [initialData])
  ...
  const handleSetCard0 = useCallback((newMedia: Media[]) => {
    setCard0(newMedia)
    if (newMedia.length > 1) {
      setCard0([newMedia[card0.length-1]])
    }
  }, [card0])

  const handleSetCard1 = useCallback((newMedia: Media[]) => {
    setCard1(newMedia)
    if (newMedia.length > 1) {
      setCard1([newMedia[card1.length-1]])
    }
  }, [card1])

  const handleSetCard2 = useCallback((newMedia: Media[]) => {
    setCard2(newMedia)
    if (newMedia.length > 1) {
      setCard2([newMedia[card2.length-1]])
    }
  }, [card2])

  const handleSetCard3 = useCallback((newMedia: Media[]) => {
    setCard3(newMedia)
    if (newMedia.length > 1) {
      setCard3([newMedia[card3.length-1]])
    }
  }, [card3])
    ...
    if (Object.keys(errors).length > 0) {
      ...
      const updatedData = {
        ...
        cards: data.cards.map((card, index) => {
          const cardImage = index === 0 ? card0 : 
                          index === 1 ? card1 :
                          index === 2 ? card2 :
                          index === 3 ? card3 : []
          const lastImage = cardImage[cardImage.length - 1]
          const imageId = typeof lastImage === 'object' && lastImage !== null ? lastImage.id : (lastImage || '')
          return {
            ...card,
            image: imageId
          }
      ...
      onSubmit(updatedData as FeaturedSectionFormValues & { cards: { image: string }[] })
    }
  }
                    ...
                    {field.value?.map((card, cardIndex) => (
                      <div key={cardIndex} className="space-y-4 p-4 border rounded-lg">
                        ...
                        <FormItem>
                          ...
                          <div className="space-y-4">
                            {cardIndex === 0 && card0 && (
                              <div className="relative h-32 w-32 overflow-hidden rounded-md bg-muted">
                                <Image
                                  resource={card0[0]}
                                  fill
                            ...
                            <ImageUploader 
                              setMedia={cardIndex === 0 ? handleSetCard0 :
                                      cardIndex === 1 ? handleSetCard1 :
                                      cardIndex === 2 ? handleSetCard2 :
                                      handleSetCard3} 
                            />
                          </div>
                        </FormItem>
                          ...
                          render={({ field }) => (
                            <FormItem>
                              <div className="space-y-1">
                                ...
                                <FormDescription>Selecione o tipo de link e o item correspondente. Isso irá definir o link que o usuário será redirecionado ao clicar no card.</FormDescription>
                              </div>
                              <div className="flex flex-col sm:flex-row gap-4">
                                <div className="w-full sm:w-1/3">
                                  <Label htmlFor="type-select" className="mb-2 block">
                                    Tipo de Link
                                  </Label>
                                  <Select
                                ...
                                <div className="w-full sm:w-2/3">
                                  <Label htmlFor="item-select" className="mb-2 block">
                                    {field.value.relationTo === "categories" ? "Categoria" : "Produto"}
                                  </Label>
                                  <Popover 
                                    ...
                                    <PopoverTrigger asChild>
                                      ...
                                      >
                                        {isLoading ? (
                                          ...
                                          getComboboxItems(field.value.relationTo).find((item) => item.value === field.value.value)?.label
                                        ) : (
                                    ...
                                    <PopoverContent className="w-[--radix-popover-trigger-width] p-0">
                                      <Command>
                                        ...
                                        <CommandList>
                                          ...
                                          <CommandGroup>
                                            {getComboboxItems(field.value.relationTo).map((item) => (
                                              <CommandItem
                                                key={item.value}
                                                value={item.value}
                                                onSelect={(currentValue) => {
                                                  field.onChange({
                                                    ...field.value,
                                                    value: currentValue === field.value.value ? "" : currentValue
                                                  })
                                                  toggleOpen(cardIndex)
                                                }}
                                              >
                                                <Check className={cn("mr-2 h-4 w-4", field.value.value === item.value ? "opacity-100" : "opacity-0")} />
                                                {item.label}
                                              </CommandItem>
                                            ))}
                                          </CommandGroup>
